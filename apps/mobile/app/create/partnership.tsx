...
/**
 * A new partnership, with the few fields worth capturing on a phone. Stages,
...
export default function CreatePartnershipScreen() {
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      await api.createPipelineEntry('partnership', {
        companyId,
        name: name.trim(),
        estimatedValue: amount ? Number(amount) : 0,
  ...
  return (
    <Screen>
      <View style={{ gap: theme.spacing.lg }}>
        ...
        <CompanyPicker value={companyId} onChange={setCompanyId} permission="partnership:write" />
